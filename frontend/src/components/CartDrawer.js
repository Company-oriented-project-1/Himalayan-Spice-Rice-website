...
import { ShoppingCart, X, ShoppingBag, Minus, Plus, Trash2, ShieldCheck } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div className="absolute inset-y-0 right-0 max-w-md w-full flex">
        <div className="w-full h-full flex flex-col bg-white shadow-2xl transform transition-transform duration-300 ease-in-out translate-x-0">
          ...
          <div className="flex items-center justify-between p-5 border-b border-stone-100 bg-stone-50/50">
            <div className="flex items-center gap-2">
              ...
              <Badge className="bg-red-100 text-red-800 ml-2">{cart.length} items</Badge>
            </div>
            <Button variant="ghost" size="icon" onClick={() => setIsCartOpen(false)} className="-mr-2">
              <X size={24} />
            </Button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-5">
            {cart.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center space-y-4 text-stone-500">
                ...
                <p className="text-sm">Looks like you havent added any authentic spices or rice yet.</p>
                    ...
                    <div className="flex flex-1 flex-col">
                      <div className="flex justify-between text-base font-medium text-stone-800">
                        ...
                        <div className="text-right ml-4">
                          ...
                          <p className="font-bold text-red-800">{formatPrice((item.discountedPrice || item.price) * item.qty)}</p>
                        </div>
                      </div>
                      <p className="mt-1 text-sm text-stone-500">{item.weight}</p>
                      <div className="flex items-center justify-between flex-1 mt-2">
                        <div className="flex items-center border border-stone-200 rounded-lg">
                          <button onClick={() => updateQuantity(item.id, -1)} className="p-1 text-stone-500 hover:text-red-800 hover:bg-stone-50 rounded-l-lg transition-colors" disabled={item.qty <= 1}><Minus size={16} /></button>
                          <span className="px-3 py-1 text-sm font-semibold text-stone-800 min-w-[2rem] text-center">{item.qty}</span>
                          <button onClick={() => updateQuantity(item.id, 1)} className="p-1 text-stone-500 hover:text-red-800 hover:bg-stone-50 rounded-r-lg transition-colors" disabled={item.qty >= item.stock}><Plus size={16} /></button>
                        </div>
                        <button type="button" onClick={() => removeFromCart(item.id)} className="font-medium text-stone-400 hover:text-red-600 transition-colors p-2">
                          <Trash2 size={18} />
                        </button>
                      </div>
                    </div>
          ...
          {cart.length > 0 && (
            <div className="border-t border-stone-100 bg-stone-50 p-5 space-y-4">
              ...
              <div className="grid grid-cols-1 gap-3">
                <Button variant="primary" size="lg" className="w-full text-base" onClick={() => {
                  setIsCartOpen(false);
                  router.push('/checkout');
                }}>
                  Proceed to Pickup Request
                </Button>
                <button onClick={() => setIsCartOpen(false)} className="text-sm font-medium text-stone-500 hover:text-stone-800 mt-2 text-center">
