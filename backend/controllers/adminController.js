...
exports.createUser = async (req, res) => {
	try {
		...
		const finalStatus = status && VALID_USER_STATUSES.includes(status) ? status : 'ACTIVE';
...
exports.updateUser = async (req, res) => {
	try {
		...
		if (err.message === 'Status must be ACTIVE, INACTIVE, BLOCKED, or DELETED') {
...
exports.updateUserStatus = async (req, res) => {
	try {
		...
		if (!status || !VALID_USER_STATUSES.includes(status)) {
			return res.status(400).json({ message: 'Status must be ACTIVE, INACTIVE, BLOCKED, or DELETED' });
		}

		if (req.user.id === id && status === 'DELETED') {
			return res.status(400).json({ message: 'Admin cannot set their own account to DELETED' });
		}
	...
	try {
		...
		if (status) {
			if (!VALID_CATEGORY_STATUSES.includes(status)) {
				return res.status(400).json({ message: 'Invalid category status filter' });
			}
...
exports.createCategoryAdmin = async (req, res) => {
	try {
		...
		if (!uniqueSlug) {
			return res.status(400).json({ message: 'Unable to generate a valid slug for this category' });
		}
...
exports.createProductAdmin = async (req, res) => {
	try {
		...
		const parsedSalePrice = salePrice === undefined || salePrice === null || salePrice === ''
			? null
			: parseNumber(salePrice);
		if (parsedSalePrice !== null && (parsedSalePrice < 0 || parsedSalePrice > parsedPrice)) {
			return res.status(400).json({ message: 'Sale price must be between 0 and the original price' });
		}

		const parsedStock = stock === undefined || stock === '' ? 0 : parseInteger(stock);
		if (parsedStock === null || parsedStock < 0) {
			return res.status(400).json({ message: 'Stock must be a valid non-negative integer' });
		}

		const parsedQuantity = quantity === undefined || quantity === '' ? 1 : parseInteger(quantity);
		if (parsedQuantity === null || parsedQuantity < 1) {
			return res.status(400).json({ message: 'Quantity must be a valid integer greater than or equal to 1' });
		}

		const normalizedStatus = status && VALID_PRODUCT_STATUSES.includes(status) ? status : 'ACTIVE';
		if (discountType !== undefined && discountType !== null && !VALID_DISCOUNT_TYPES.includes(discountType)) {
			return res.status(400).json({ message: 'Discount type must be NONE, PERCENTAGE, or FLAT' });
		}

		const normalizedDiscountType = discountType && VALID_DISCOUNT_TYPES.includes(discountType)
			? discountType
			: 'NONE';

		if (normalizedDiscountType !== 'NONE' && parsedSalePrice === null) {
			return res.status(400).json({ message: 'Sale price is required when discount type is PERCENTAGE or FLAT' });
		}

		const normalizedUnit = String(unit || 'kg').trim() || 'kg';
		const normalizedProductCode = productCode === undefined || productCode === null
			? null
		...
		if (!uniqueSlug) {
			return res.status(400).json({ message: 'Unable to generate a valid slug for this product' });
		}
		...
		if (err.code === 'P2002') {
			if (Array.isArray(err.meta?.target) && err.meta.target.includes('productCode')) {
...
exports.getProductByCodeAdmin = async (req, res) => {
	try {
		...
		res.status(500).json({ message: 'Failed to fetch product by code', error: err.message });
	}
};
...
exports.updateProductAdmin = async (req, res) => {
	try {
		...
		if (salePrice !== undefined) {
			if (salePrice === null || salePrice === '') {
				...
				if (parsedSalePrice === null || parsedSalePrice < 0) {
					return res.status(400).json({ message: 'Sale price must be a valid non-negative number' });
				}
		...
		if (images !== undefined) {
			...
			data.images = images.map((item) => String(item || '').trim()).filter(Boolean);
		}
		...
		const nextPrice = data.price !== undefined ? data.price : Number(existingProduct.price);
		const nextSalePrice = data.salePrice !== undefined
			? data.salePrice
			: existingProduct.salePrice === null || existingProduct.salePrice === undefined
				? null
				: Number(existingProduct.salePrice);
		const nextDiscountType = data.discountType !== undefined
			? data.discountType
			: existingProduct.discountType || 'NONE';

		if (nextSalePrice !== null && nextSalePrice > nextPrice) {
			return res.status(400).json({ message: 'Sale price must be less than or equal to the original price' });
		}
...
exports.getHeroSlidersAdmin = async (req, res) => {
	try {
		...
		res.status(500).json({ message: 'Failed to fetch hero sliders', error: err.message });
	}
};
...
exports.createHeroSliderAdmin = async (req, res) => {
	try {
		...
		const normalizedPriority = priority === undefined || priority === '' ? 0 : parseInteger(priority);
		...
		if (status !== undefined && status !== '' && !VALID_HERO_SLIDER_STATUSES.includes(status)) {
			return res.status(400).json({ message: 'Status must be ACTIVE or INACTIVE' });
		}
		...
		res.status(201).json({ message: 'Hero slider created successfully', slider });
	} catch (err) {
		res.status(500).json({ message: 'Failed to create hero slider', error: err.message });
	}
};
...
exports.getHeroSliderByIdAdmin = async (req, res) => {
	try {
		...
		res.status(500).json({ message: 'Failed to fetch hero slider', error: err.message });
	}
};
...
exports.updateHeroSliderAdmin = async (req, res) => {
	try {
		const { id } = req.params;
		const existingSlider = await prisma.heroSlider.findUnique({ where: { id } });
		if (!existingSlider) {
			return res.status(404).json({ message: 'Hero slider not found' });
		}
		...
		res.status(200).json({ message: 'Hero slider updated successfully', slider });
	} catch (err) {
		res.status(500).json({ message: 'Failed to update hero slider', error: err.message });
	}
};
...
exports.updateHeroSliderStatusAdmin = async (req, res) => {
	try {
		...
		res.status(200).json({ message: 'Hero slider status updated successfully', slider: updatedSlider });
	} catch (err) {
		res.status(500).json({ message: 'Failed to update hero slider status', error: err.message });
	}
};
	...
	try {
		...
		if (storeName !== undefined) {
			...
			if (normalizedStoreName.length < 2) {
				return res.status(400).json({ message: 'Store name must be at least 2 characters long' });
			}
		...
		if (deliveryFee !== undefined) {
			...
			if (normalizedDeliveryFee === null || normalizedDeliveryFee < 0) {
				return res.status(400).json({ message: 'Delivery fee must be a valid non-negative number' });
			}
		...
		if (minOrderForFreeDelivery !== undefined) {
			...
			if (normalizedMinOrder === null || normalizedMinOrder < 0) {
				return res.status(400).json({ message: 'Minimum order for free delivery must be a valid non-negative number' });
			}
		...
		if (isStoreOpen !== undefined) {
			...
			if (typeof parsedStoreOpen !== 'boolean') {
				return res.status(400).json({ message: 'isStoreOpen must be true or false' });
			}
